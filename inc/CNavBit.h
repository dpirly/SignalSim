...
private:
	...
	unsigned int EphMessage[42][2][9];	// 32 GPS + 10 QZSS SVs, Message 10/11 each has 276bits from bit19 DWORD0 to bit0 DWORD8
	...
	unsigned int ClockMessage[42][4];	// 32 GPS + 10 QZSS SVs, Clock fields (top through af2) 89bits from bit13 DWORD1 to bit21 DWORD4
	unsigned int DelayMessage[42][3];	// 32 GPS + 10 QZSS SVs, Group delay fields 65bits from bit20 DWORD4 to bit20 DWORD6
	...
	unsigned char ConvEncodeBitsL2[42];
	unsigned char ConvEncodeBitsL5[42];

	int ComposeEphWords(PGPS_EPHEMERIS Ephemeris, unsigned int EphData[2][9], unsigned int ClockData[4], unsigned int DelayData[3], bool IsQzss);
